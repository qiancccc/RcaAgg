...
class ImageHelper(Helper):
    ...
    def load_data(self):
        ...
        if self.params['dataset'] == 'fmnist':
            self.train_dataset = datasets.FashionMNIST(self.params['data_folder'], train=True, download=True,
                                                       transform=transforms.Compose([
                                                           transforms.ToTensor(),
                                                           transforms.Normalize((0.1307,), (0.3081,))
                                                       ]))
            self.test_dataset = datasets.FashionMNIST(self.params['data_folder'], train=False, download=True,
                                                      transform=transforms.Compose([
                                                          transforms.ToTensor(),
                                                          transforms.Normalize((0.1307,), (0.3081,))
                                                      ]))
    ...
    def poison_dataset(self):
        ...
        if args.attack_mode.lower() in ['mr', 'dba', 'flip', 'combine']:
            ...
            print(f"A T T A C K - M O D E: C O M B I N E 2 !")
            range_no_id = list(range(len(self.test_dataset)))
            # 只移除前三个目标标签（poison_label_swaps 的前3个）
            for label in self.params['poison_label_swaps'][:3]:
                remove_no_id = self.sample_poison_data(label)
                range_no_id = list(set(range_no_id) - set(remove_no_id))
            # 注意：不移除 poison_label_swap 对应的标签样本
            indices = []
            while len(indices) < self.params['size_of_secret_dataset']:
                range_iter = random.sample(range_no_id, self.params['batch_size'])
                indices.extend(range_iter)
            ...
            elif args.attack_mode.lower() == 'neurotoxin':
            ...
            if evaluation:
                if args.attack_mode.lower() == 'mr':
                    ...
                    if adversarial_index == 0:
                        ...
                        raise ValueError('Unrecognized Adversarial Index for COMBINE2')
            else:
                if args.attack_mode.lower() == 'mr':
                    if args.dataset == 'cifar10':
                        new_inputs[index] = self.train_dataset[
